...
import { useI18n } from "@/hooks/useI18n";
import AuthHeroSection from "@/components/auth/AuthHeroSection";
import { Upload, ArrowRight, ArrowLeft, User, Phone, Mail, Lock, ShieldCheck, Camera } from "lucide-react";
import { useAuth } from "@/contexts/AuthContext";
import { Button } from "@/components/ui/Button";
import { toast } from "sonner";
...
const SignUpPage = () => {
  ...
  const validateStep1 = () => {
    ...
    if (formData.username.length < 3) newErrors.username = "Pseudo trop court";
  ...
  const handleNext = () => {
    ...
    else toast.error("Veuillez compléter tous les champs obligatoires.");
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await register({
        ...formData,
        roles: [formData.role === "admin" ? "FLEET_ADMIN" : formData.role === "driver" ? "FLEET_DRIVER" : "FLEET_MANAGER"],
        file: file || undefined,
      });
      toast.success("Compte créé !");
    } catch (err: any) {
      toast.error(err.title || "Erreur lors de l'inscription");
    }
  };

  const inputStyles = (name: string) => `
    w-full h-12 pl-11 pr-4 rounded-xl border text-sm transition-all duration-200 outline-none
    ${errors[name] 
      ? "border-red-500 bg-red-50/30 dark:bg-red-900/10 focus:ring-2 focus:ring-red-500/20" 
      : "border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800/50 focus:border-primary focus:ring-4 focus:ring-primary/10"}
    dark:text-white placeholder:text-slate-400
  `;

  return (
    <div className="flex min-h-screen w-full bg-white dark:bg-slate-950 overflow-hidden">
      <AuthHeroSection 
        titleKey="heroTitle" descriptionKey="heroDetails"
        imageUrl="https://images.unsplash.com/photo-1601584115197-04ecc0da31d7?q=80&w=2070&auto=format&fit=crop"
        altText="Logistics" stats={[{ icon: "hub", label: "signupHeroStatLabel", description: "signupHeroStatDescription" }]}
      />

      <div className="flex w-full lg:w-1/2 flex-col h-screen overflow-y-auto">
        <div className="flex-grow flex flex-col justify-center px-6 py-12 sm:px-16 xl:px-24">
          <div className="mx-auto w-full max-w-md">
            
            {/* Header & Steps */}
            <div className="mb-10 text-center lg:text-left">
              <div className="flex items-center justify-center lg:justify-start gap-2 mb-6">
                <span className={`h-1.5 w-12 rounded-full transition-all duration-300 ${step === 1 ? "bg-primary" : "bg-primary/30"}`} />
                <span className={`h-1.5 w-12 rounded-full transition-all duration-300 ${step === 2 ? "bg-primary" : "bg-slate-100 dark:bg-slate-800"}`} />
              </div>
              <h2 className="text-3xl font-black text-slate-900 dark:text-white tracking-tight">
                {step === 1 ? "Rejoignez-nous" : "Finalisez votre accès"}
              </h2>
              <p className="text-slate-500 dark:text-slate-400 mt-2 text-sm font-medium">
                {step === 1 ? "Commençons par faire connaissance." : "Configurez vos identifiants de connexion."}
              </p>
            </div>
                ...
                {step === 1 ? (
                  <motion.div key="st1" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-4">
                    ...
                    <div className="flex flex-col items-center pb-4">
                      <div className="relative group">
                        <div className="w-24 h-24 rounded-full border-2 border-dashed border-slate-300 dark:border-slate-700 flex items-center justify-center bg-slate-50 dark:bg-slate-900 overflow-hidden transition-all group-hover:border-primary">
                          {file ? (
                      ...
                      <span className="text-[11px] font-bold uppercase tracking-wider text-slate-400 mt-3">Photo de profil</span>
                    </div>

                    <div className="grid grid-cols-2 gap-4">
                      <div className="relative">
                        <User className="absolute left-4 top-3.5 w-4 h-4 text-slate-400" />
                        <input name="firstName" value={formData.firstName} onChange={handleChange} className={inputStyles("firstName")} placeholder="Prénom (ex: Jean)" />
                      </div>
                      <div className="relative">
                        <User className="absolute left-4 top-3.5 w-4 h-4 text-slate-400" />
                        <input name="lastName" value={formData.lastName} onChange={handleChange} className={inputStyles("lastName")} placeholder="Nom (ex: Dupont)" />
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="relative">
                        ...
                        <input name="username" value={formData.username} onChange={handleChange} className={inputStyles("username")} placeholder="Pseudo" />
                      </div>
                      <div className="relative">
                        <Phone className="absolute left-4 top-3.5 w-4 h-4 text-slate-400" />
                        <input name="phone" value={formData.phone} onChange={handleChange} className={inputStyles("phone")} placeholder="Téléphone" />
                      </div>
                    </div>

                    <div className="relative">
                      <ShieldCheck className="absolute left-4 top-3.5 w-4 h-4 text-slate-400" />
                      <select name="role" value={formData.role} onChange={handleChange} className={`${inputStyles("role")} appearance-none cursor-pointer`}>
                        <option value="" disabled>Sélectionnez votre rôle</option>
                        <option value="manager">Gestionnaire de flotte</option>
                        <option value="driver">Chauffeur professionnel</option>
                    ...
                    <Button type="button" onClick={handleNext} className="w-full h-12 rounded-xl text-sm font-bold shadow-xl shadow-primary/20 mt-2">
                  ...
                  <motion.div key="st2" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-4">
                    ...
                    <div className="relative">
                      ...
                      <input name="email" type="email" value={formData.email} onChange={handleChange} className={inputStyles("email")} placeholder="Email professionnel (ex: contact@entreprise.com)" autoComplete="email" />
                      {errors.email && <span className="text-[10px] text-red-500 ml-1 mt-1">{errors.email}</span>}
                    </div>

                    <div className="relative">
                      <Lock className="absolute left-4 top-3.5 w-4 h-4 text-slate-400" />
                      <input 
                        name="password" type={passwordVisible ? "text" : "password"} value={formData.password} onChange={handleChange} 
                        className={inputStyles("password")} placeholder="Définir un mot de passe" autoComplete="new-password"
                      />
                      <button type="button" onClick={() => setPasswordVisible(!passwordVisible)} className="absolute right-4 top-3 text-slate-400 hover:text-primary transition-colors">
                        <span className="material-symbols-outlined text-lg">{passwordVisible ? "visibility_off" : "visibility"}</span>
                      </button>
                    </div>
                    ...
                    <div className="relative">
                      <Lock className="absolute left-4 top-3.5 w-4 h-4 text-slate-400" />
                      <input 
                        name="confirmPassword" type={passwordVisible ? "text" : "password"} value={formData.confirmPassword} onChange={handleChange} 
                        className={inputStyles("confirmPassword")} placeholder="Confirmer le mot de passe" autoComplete="new-password"
                      />
                    </div>

                    <div className="flex gap-3 pt-4">
                      <button type="button" onClick={() => setStep(1)} className="flex items-center justify-center px-4 rounded-xl border border-slate-200 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-900 transition-all">
                        <ArrowLeft className="w-5 h-5 text-slate-500" />
                      </button>
                      <Button type="submit" isLoading={isLoading} className="flex-grow h-12 rounded-xl text-sm font-bold shadow-xl shadow-primary/20">
                        Créer mon compte
                      </Button>
                    </div>
                  </motion.div>
            ...
            <p className="mt-10 text-center text-slate-500 dark:text-slate-400 text-sm">
              Déjà inscrit ? <Link className="font-bold text-primary hover:underline ml-1" href={`/${locale}/login`}>Se connecter</Link>
            </p>
          </div>
        </div>
      </div>
