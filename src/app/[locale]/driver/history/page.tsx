...
import {
  mockDriverMissions,
  mockDriverHistorySummary,
  DriverMission,
} from "@/data/mockDriverHistory";
...
const DriverHistoryPage = () => {
  ...
  const [selectedMission, setSelectedMission] = useState<DriverMission | null>(
    null,
  );
  ...
  const handleExport = () => {
    ...
    const csvContent = filteredMissions
      .map((mission) => {
        const statusText =
      ...
      <div className="max-w-[1440px] w-full mx-auto px-6 py-8 flex flex-col gap-6">
        <DashboardHeader
          ...
          onNotificationsClick={() => {
            /* Placeholder for notification modal */
          }}
        />
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex flex-col gap-1">
            <h2 className="text-3xl font-black tracking-tight">
              {t("historyTitle", "driverHistoryPage")}
            </h2>
            <p className="text-slate-500 dark:text-[#9dabb9] text-base">
              {t("historySubtitle", "driverHistoryPage")}
            </p>
          </div>
          <div className="flex gap-3">
            ...
            >
              ...
              <option value="lastMonth">
                {t("lastMonth", "driverHistoryPage")}
              </option>
              <option value="custom">
                {t("customRange", "driverHistoryPage")}
              </option>
            </select>
            {dateFilterType === "custom" && (
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 h-10 rounded-lg bg-primary text-white text-sm font-bold hover:bg-blue-600 transition-colors shadow-sm"
            >
        ...
        <SummaryCards
          distance={{
            value: mockDriverHistorySummary.totalDistance,
            trend: "8.2",
          }}
          trips={{ value: mockDriverHistorySummary.completedTrips, trend: "4" }}
          score={{
            value: Math.round(mockDriverHistorySummary.fuelEfficiency),
            trend: "0",
          }}
        />
        ...
        <div className="flex justify-end">
          <div className="flex gap-2">
            <button
              ...
              className="px-3 py-1 rounded border border-slate-200 dark:border-slate-700 text-sm text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-[#283039] disabled:opacity-50"
              disabled={currentPage === 1}
            >
              {t("previous", "driverHistoryPage")}
            </button>
            <button
              onClick={() => paginate(currentPage + 1)}
              className="px-3 py-1 rounded border border-slate-200 dark:border-slate-700 text-sm text-slate-900 dark:text-white hover:bg-slate-50 dark:hover:bg-[#283039]"
              disabled={currentPage === totalPages}
            >
              {t("next", "driverHistoryPage")}
            </button>
          </div>
        </div>
      </div>
