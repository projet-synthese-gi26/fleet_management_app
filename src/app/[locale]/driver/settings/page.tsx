...
const SettingsCard = ({ title, icon, children }: { title: string, icon: React.ReactNode, children: React.ReactNode }) => (
    <div className="bg-white dark:bg-gray-800/50 rounded-lg shadow-sm overflow-hidden border border-gray-200 dark:border-gray-700">
    ...
    return (
        <div className="p-4 md:p-6 lg:p-8 space-y-8 max-w-4xl mx-auto">
            <header>
                ...
                <p className="mt-1 text-gray-600 dark:text-gray-400">Manage your account, preferences, and notifications.</p>
            </header>

            <SettingsCard title="Profile" icon={<UserIcon className="w-6 h-6 text-gray-500 dark:text-gray-400" />}>
            ...
            <SettingsCard title="Appearance & Language" icon={<PaletteIcon className="w-6 h-6 text-gray-500 dark:text-gray-400" />}>
            ...
            <SettingsCard title="Notifications" icon={<BellIcon className="w-6 h-6 text-gray-500 dark:text-gray-400" />}>
                <div className="space-y-5">
                    <div className="flex items-center justify-between">
                        <p className="font-medium text-gray-900 dark:text-white">New Mission Alerts</p>
                        <Switch id="newMission" checked={notifications.newMission} onChange={() => handleNotificationChange('newMission')} />
                    </div>
                    <div className="flex items-center justify-between">
                        <p className="font-medium text-gray-900 dark:text-white">Mission Status Updates</p>
                        <Switch id="missionUpdates" checked={notifications.missionUpdates} onChange={() => handleNotificationChange('missionUpdates')} />
                    </div>
                    <div className="flex items-center justify-between">
                        <p className="font-medium text-gray-900 dark:text-white">Document Expiration Reminders</p>
            ...
            <SettingsCard title="Security" icon={<LockIcon className="w-6 h-6 text-gray-500 dark:text-gray-400" />}>
                <div className="flex items-center justify-between">
                    <div>
