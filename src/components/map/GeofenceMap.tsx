...
import { MapContainer, TileLayer, FeatureGroup, Circle, Polygon, useMap } from "react-leaflet";
...
interface GeofenceMapProps {
  existingZones: Zone[];
  onZoneCreated: (zoneData: Omit<CreateZoneDto, 'fleetId' | 'name' | 'description'>) => void;
...
export default function GeofenceMap({ existingZones, onZoneCreated, onZoneDeleted }: GeofenceMapProps) {
  ...
  return (
    <MapContainer center={defaultCenter} zoom={12} style={{ height: "100%", width: "100%" }}>
      ...
      {existingZones.map((zone) => {
        ...
        } else if (zone.type === 'CIRCLE' && zone.vertices.length > 0 && zone.radius) {
